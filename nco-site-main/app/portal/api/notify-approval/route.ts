// app/portal/api/notify-approval/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    const { eventId, filePath, note }: Body = await req.json();

    if (!eventId || !filePath) {
      return NextResponse.json({ error: 'Missing eventId or filePath' }, { status: 400 });
    }

    // Only admins or event organizers can notify
    ...
    // Try to enrich the email with event title/slug
    let title = eventId;
    ...
    const subject = `APPROVED: Demo Bag for ${title}`;
    const html = `
      <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;line-height:1.5;color:#111">
        <h2 style="margin:0 0 8px">Demo Bag Approved</h2>
        <p style="margin:0 0 8px"><strong>Event:</strong> ${title}</p>
        <p style="margin:0 0 8px"><strong>File:</strong> ${filePath}</p>
        ${note ? `<p style="margin:0 0 8px"><strong>Note:</strong> ${note}</p>` : ''}
