...
export default function Client({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/portal/api/events/by-slug/${encodeURIComponent(slug)}`)
      ...
      .catch((err) => { if (!alive) return; console.error(err); setLoading(false); setEvent(null) })
    return () => { alive = false }
  }, [slug])
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="text-2xl font-semibold">{loading ? 'Loading…' : event ? event.title : 'Event not found'}</h1>
          {!loading && event && (
      ...
      {/* Content */}
      <div className="mt-6">
        {loading && <div className="flex items-center gap-3 text-gray-600"><Spinner /> <span>Loading event…</span></div>}
        {!loading && !event && <div className="rounded-xl border bg-white p-6 text-center text-gray-600">Event not found.</div>}
        {!loading && event && (
          <>
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div className="rounded-xl border bg-white p-6 lg:col-span-2">
        ...
        <dl className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div><dt className="text-gray-500">Title</dt><dd className="font-medium">{event.title}</dd></div>
          <div><dt className="text-gray-500">Slug</dt><dd className="font-medium">{event.slug ?? '—'}</dd></div>
          <div><dt className="text-gray-500">Date</dt><dd className="font-medium">{fmtDate(event.date)}</dd></div>
          <div><dt className="text-gray-500">City</dt><dd className="font-medium">{event.city ?? 'TBD'}</dd></div>
        </dl>
      </div>
      <div className="rounded-xl border bg-white p-6">
        <h3 className="text-sm font-semibold text-gray-700">Quick Actions</h3>
        <div className="mt-3 grid gap-2">
          <a href={`/portal/events/${event.slug ?? event.id}`} className="rounded-lg bg-usaBlue text-white px-3 py-2 text-sm text-center hover:opacity-90">
            View Public Page
          </a>
        </div>
      </div>
...
function RegistrationsTab({ eventId, onToast }: { eventId: string; onToast: (t: { msg: string; kind: 'success' | 'error' }) => void }) {
  ...
  function exportCSV() {
    ...
    filtered.forEach((r) => {
      lines.push([r.id ?? '', r.userId ?? '', (r.user?.name ?? '').replaceAll(',', ' '), (r.user?.email ?? '').replaceAll(',', ' '), r.createdAt ?? ''].join(','))
    })
  ...
  return (
    <div className="rounded-xl border bg-white p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <input placeholder="Search name, email, userId…" value={q} onChange={(e) => setQ(e.target.value)} className="w-full sm:w-64 rounded border px-3 py-2 text-sm" aria-label="Search registrations" />
          <button onClick={exportCSV} className="rounded border px-3 py-2 text-sm hover:bg-gray-50">Export CSV</button>
        </div>
      </div>

      <div className="mt-4">
        <form onSubmit={addRegistration} className="flex flex-col sm:flex-row gap-2">
          <input value={userId} onChange={(e) => setUserId(e.target.value)} className="rounded border px-3 py-2 text-sm" placeholder="user id (e.g., demo-user-1)" />
          <button disabled={adding} className="inline-flex items-center gap-2 rounded bg-usaBlue text-white px-3 py-2 text-sm hover:opacity-90 disabled:opacity-60">
...
function SponsorsTab({ event, onToast }: { event: Event; onToast: (t: { msg: string; kind: 'success' | 'error' }) => void }) {
  const [rows, setRows] = useState<Array<{ id: string; name: string; url?: string; logo?: string }>>([])
  const [name, setName] = useState(''); const [url, setUrl] = useState(''); const [logo, setLogo] = useState('')

  function addSponsor(e: React.FormEvent) {
    e.preventDefault()
    if (!name.trim()) return onToast({ msg: 'Sponsor name required', kind: 'error' })
    const row = { id: crypto.randomUUID(), name: name.trim(), url: url || undefined, logo: logo || undefined }
    setRows((prev) => [row, ...prev]); setName(''); setUrl(''); setLogo('')
    onToast({ msg: 'Sponsor added (local only)', kind: 'success' })
  }

  return (
    <div className="rounded-xl border bg-white p-6">
      <h2 className="text-lg font-semibold">Sponsors</h2>
      <p className="mt-1 text-sm text-gray-600">Attach sponsors to <span className="font-medium">{event.title}</span>. (Local state for now — API soon.)</p>
      <form onSubmit={addSponsor} className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
        <input className="rounded border px-3 py-2 text-sm" placeholder="Name *" value={name} onChange={(e)=>setName(e.target.value)} />
        <input className="rounded border px-3 py-2 text-sm" placeholder="URL (optional)" value={url} onChange={(e)=>setUrl(e.target.value)} />
        <input className="rounded border px-3 py-2 text-sm" placeholder="Logo URL (optional)" value={logo} onChange={(e)=>setLogo(e.target.value)} />
        <div className="sm:col-span-3">
          <button className="rounded bg-usaBlue text-white px-3 py-2 text-sm hover:opacity-90">Add Sponsor</button>
        </div>
      </form>
      <div className="mt-6 overflow-hidden rounded-lg border">
        <div className="hidden sm:grid grid-cols-12 gap-3 px-4 py-3 border-b bg-gray-50 text-xs font-semibold text-gray-600">
              ...
              <li key={s.id} className="px-4 py-3">
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center">
                  ...
                  <div className="col-span-2 text-right">
                    <button onClick={()=>{ setRows(prev => prev.filter(x=>x.id!==s.id)); onToast({ msg:'Removed', kind:'success'}) }} className="rounded border px-3 py-1 text-sm hover:bg-gray-50">Remove</button>
                  </div>
                </div>
              </li>
            ))}
