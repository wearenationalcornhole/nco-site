...
export default function OrganizerDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Gate on client only (no server cookies())
        ...
        // Load events via API to avoid any server auth helpers
        ...
        // Count registrations for first 10 to keep things snappy
        const top = list.slice(0, 10);
        const counts = await Promise.all(top.map(async (e) => {
          try {
  ...
  if (loading) return <main className="py-8">Loading…</main>;
  if (error) return (
    <main className="py-8">
  ...
  const now = new Date();
  const upcoming = events.filter(e => !e.date || new Date(e.date + 'T00:00:00Z') >= now);
  const past = events.filter(e => e.date && new Date(e.date + 'T00:00:00Z') < now);
    ...
    <main className="py-8">
      ...
      <section className="mt-8 rounded-2xl border bg-white overflow-hidden">
        <div className="px-4 py-3 border-b bg-gray-50 flex items-center justify-between">
          ...
          <Link href="/portal/org/events" className="text-sm text-usaBlue hover:underline">View all</Link>
        </div>
