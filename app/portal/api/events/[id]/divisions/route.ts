...
type DivisionRow = {
  id?: string            // <-- make optional so devStore.upsert can generate it
  event_id: string
  name: string
  cap?: number | null
...
export async function GET(_req: Request, context: any) {
  try {
    ...
    const out: DivisionOut[] = divisions
      ...
      .map((d) => ({
        id: d.id!, // devStore generates id on create
        name: d.name,
        cap: d.cap ?? null,
        createdAt: asIso(d.created_at),
        membersCount: countMap.get(d.id!) ?? 0,
...
export async function POST(req: Request, context: any) {
  try {
    ...
    // dev fallback (no id on payload; devStore generates it)
