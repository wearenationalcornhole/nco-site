...
// Demo identity we use until auth lands
...
export async function GET() {
  try {
    const prisma = await getPrisma() // ← IMPORTANT: await

    // ─────────────────────────────────────────────────────────────
    // Path A: Real DB (Prisma + Supabase)
    // ─────────────────────────────────────────────────────────────
    if (prisma) {
      ...
      // 2) Ensure the user is in a club (or create one)
      ...
      if (!membership) {
        // Create a club and membership for this organizer
        ...
        membership = await prisma.club_members.create({
          data: {
            user_id: user.id,
            club_id: club.id,
            role: 'organizer',
    ...
    // ─────────────────────────────────────────────────────────────
    // Path B: Dev fallback (in-memory devStore)
    // ─────────────────────────────────────────────────────────────
    // user
    ...
    if (!club) {
      ...
      devStore.upsert<any>('club_members', {
        club_id: club.id,
        user_id: user.id!,
        role: 'organizer',
      })
    ...
    return NextResponse.json({
      source: 'devStore',
      user,
      club: club ? { id: club.id, name: club.name, city: club.city, state: club.state, logo_url: club.logo_url, website: club.website } : null,
    })
  } catch (e: any) {
