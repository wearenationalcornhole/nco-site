    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main id="main" className="flex-1">
        ...
        <section id="tournaments" className="py-16 sm:py-20 bg-ncoGray">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex items-end justify-between gap-4 mb-8">
              ...
              <a href="/events" className="hidden sm:inline-block text-sm font-medium text-usaBlue hover:text-usaRed">View all</a>
            </div>
        ...
        <section id="sponsors" className="py-10 bg-white border-t">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 className="text-center text-sm font-semibold text-gray-600 uppercase tracking-wider">Sponsors &amp; Partners</h2>
            <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 items-center gap-6">
              {(sponsors as Sponsor[]).map((s) => (
                <a key={s.name} href={s.url} className="flex items-center justify-center opacity-80 hover:opacity-100 transition" target="_blank" rel="noopener noreferrer">
                  <img src={s.logo} alt={s.name} className="h-10 w-auto object-contain grayscale hover:grayscale-0" loading="lazy" />
                </a>
              ))}
            </div>
          </div>
        ...
        {/* Portal */}
        <section id="portal" className="py-16 sm:py-20 bg-usaBlue text-white">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
            <h2 className="text-3xl sm:text-4xl font-semibold">Player &amp; Tournament Portal</h2>
            <p className="mx-auto mt-3 max-w-2xl text-white/90">
              Register for events, track scores, manage teams, and access official rankings — all in one place.
            </p>
            <a href="/portal/login" className="mt-6 inline-flex items-center justify-center rounded-full bg-usaRed px-6 py-3 font-bold text-white hover:bg-[#8F1732]">
              Go to Portal
            </a>
          </div>
        </section>
